...
def tfr(timeseries, freqs, samp_freq, width=7.):
    ...
    tfr_complex = np.zeros((n_epochs, n_freqs, n_samps), dtype=complex)
    ...
    for trial_idx in np.arange(n_epochs):
        ...
        for freq_idx in np.arange(n_freqs):
            tfr_trial_freq, tfr_trial_freq_complex = _energyvec(freqs[freq_idx],
                                                                ts_detrended, samp_freq, width)
            tfr[trial_idx, freq_idx, :] = tfr_trial_freq
            tfr_complex[trial_idx, freq_idx, :] = tfr_trial_freq_complex

    return tfr, tfr_complex


def _get_power_thresholds(tfr, threshold=6., FOM_threshold=True):
    '''Get the power threshold for each frequency band of a TFR'''

    med_powers = np.median(tfr, axis=1)
    if FOM_threshold:
        thresholds = med_powers * threshold
    else:
        thresholds = np.quantile(tfr, threshold, axis=1)
    return thresholds, med_powers
...
def _energyvec(f, s, Fs, width=7.):
    ...
    fourier_coeff = np.convolve(s, m)  # Fourier coefficient for this frequency as a function of time
    fourier_coeff *= np.sqrt(2) * dt  # normalization factor
    
    lowerLimit = int(np.ceil(len(m) / 2))
    upperLimit = int(len(fourier_coeff) - np.floor(len(m) / 2) + 1)
    fourier_coeff = fourier_coeff[lowerLimit:upperLimit]  # crop
    
    y = np.abs(fourier_coeff) ** 2

    return y, fourier_coeff
